...
### Data Storage
The application uses a hybrid data storage approach. All user-specific data (profiles, collections, marketplace listings, admin credentials) are primarily stored client-side in AsyncStorage. For backend persistence, PostgreSQL is employed via Drizzle ORM, with a schema defined for users, sessions, and social features (friendships, messages). A robust card seeding mechanism populates the `pokemon_cards` table in PostgreSQL from the Pokémon TCG API and Scrydex, including comprehensive Japanese, Korean, and Chinese set data, with mechanisms for rate-limit handling and data deduplication.
