 ...
 *
 * Deduplication rules:
 *   - Sets:  skip if pokescan_sets.id already exists
 *   - Cards: skip if pokescan_cards.id already exists AND image_small is populated
 *            update image_small/image_large if the card exists but images are null
 */

import { db } from "./db";
import { pokescanSets, pokescanCards } from "@shared/schema";
...
export interface ScrydexCard {
  id: string;       // e.g. "sv10-3" — matches pokescan_cards.id
 ...
 *   - If the set is NOT in pokescan_sets → insert it
 *   - Then fetch all cards for that set from scrydex
 *     - If a card is NOT in pokescan_cards → insert it
 *     - If a card IS in pokescan_cards but image_small is NULL → update images
 *
...
export async function runScrydexSync(
  ...
  try {
    ...
    const existingSetRows = await db
      .select({ id: pokescanSets.id, cardCount: count(pokescanCards.id) })
      .from(pokescanSets)
    ...
    for (const set of setsToProcess) {
      ...
      if (!existingSetIds.has(setId)) {
        try {
          ...
          await db.insert(pokescanSets).values({
            id: detail.id,
            name: detail.name,
      ...
      try {
        ...
        const existingCardsRes = await db
          .select({ id: pokescanCards.id, imageSmall: pokescanCards.imageSmall })
          .from(pokescanCards)
          .where(inArray(pokescanCards.id, cardIds));
        ...
        for (const card of cards) {
          ...
          if (!existingCards.has(card.id)) {
            ...
            if (!existingImg || existingImg === "") {
              try {
                await db.update(pokescanCards)
                  .set({ imageSmall: card.imageSmall, imageLarge: card.imageLarge })
                  .where(
                    and(
                      eq(pokescanCards.id, card.id),
                      or(isNull(pokescanCards.imageSmall), eq(pokescanCards.imageSmall, ""))
                    )
                  );
