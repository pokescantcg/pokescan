...
async function syncCardsForSet(setId: string, setName: string, force = false): Promise<number> {
  ...
  if (!force && allCards.length > 0) {
    ...
    const existing = await db
      ...
      .from(pokescanCards)
      .where(inArray(pokescanCards.id, ids));
...
export async function runFullSync(force = false): Promise<void> {
  ...
  try {
    ...
    const sets = await db.select().from(pokescanSets);
...
export async function runPriceRefresh(): Promise<void> {
  ...
  try {
    const allSets = await db.select({ id: pokescanSets.id, name: pokescanSets.name }).from(pokescanSets);

    for (const set of allSets) {
      const cards = await db
        .select({
          id: pokescanCards.id,
          name: pokescanCards.name,
          number: pokescanCards.number,
          rarity: pokescanCards.rarity,
          setId: pokescanCards.setId,
        })
        .from(pokescanCards)
        .where(eq(pokescanCards.setId, set.id));
...
async function runFastCardSeed(): Promise<void> {
  ...
  const alreadySeededRows = await db
    .select({ setId: pokescanCards.setId })
    .from(pokescanCards)
    .groupBy(pokescanCards.setId);
...
async function runScrydexStartupSync(): Promise<void> {
  try {
    ...
    const emptyRes = await db.execute(
      sql`SELECT COUNT(*) AS cnt FROM pokescan_sets s
          WHERE NOT EXISTS (SELECT 1 FROM pokescan_cards c WHERE c.set_id = s.id)`
    );
  ...
  setTimeout(async () => {
    try {
      const [totalSetRows, seededSetRows] = await Promise.all([
        db.select({ count: sql<number>`count(*)::int` }).from(pokescanSets),
        db.select({ count: sql<number>`count(distinct set_id)::int` }).from(pokescanCards),
      ]);
