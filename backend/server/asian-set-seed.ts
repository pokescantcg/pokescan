...
const JAPANESE_SETS: Array<{ slug: string; id: string }> = [
...
// ─── Korean Sets ──────────────────────────────────────────────────────────────
// Korean TCG closely mirrors Japanese releases. All use _ko suffix.

interface KoSet { id: string; name: string; releaseDate?: string }

const KOREAN_SETS: KoSet[] = [
...
// ─── Chinese (Simplified) Sets ────────────────────────────────────────────────
// Pokémon TCG China started 2013. Main sets use _zh suffix.

interface ZhSet { id: string; name: string; releaseDate?: string }

const CHINESE_SETS: ZhSet[] = [
  // Scarlet & Violet era (2023–present)
...
export interface AsianSeedResult {
  inserted: number;
  skipped: number;
  errors: number;
  details: string[];
...
async function upsertSet(
  ...
  try {
    await pool.query(
      `INSERT INTO pokescan_sets (id, name, series, printed_total, total, release_date, logo_url, symbol_url)
...
async function getExistingSetIds(): Promise<Set<string>> {
  const result = await pool.query("SELECT id FROM pokescan_sets");
  return new Set(result.rows.map((r: any) => r.id));
}

export async function seedAsianSets(
  onProgress?: (msg: string) => void
): Promise<AsianSeedResult> {
  const result: AsianSeedResult = { inserted: 0, skipped: 0, errors: 0, details: [] };
  ...
  // ── Japanese sets ──
  log(`Inserting ${JAPANESE_SETS.length} Japanese sets...`);
  for (const { slug, id } of JAPANESE_SETS) {
    ...
    if (outcome === "inserted") { result.inserted++; log(`✓ JP: ${name} (${id})`); }
    else if (outcome === "error") result.errors++;
  }

  // ── Korean sets ──
  log(`Inserting ${KOREAN_SETS.length} Korean sets...`);
  for (const set of KOREAN_SETS) {
    if (existing.has(set.id)) { result.skipped++; continue; }
    const series = inferSeries(set.id);
    // Try to map to a Scrydex image (using base JP slug pattern won't work, but shows something)
    const outcome = await upsertSet(set.id, set.name, series, set.releaseDate || "", "", "", 0);
    if (outcome === "inserted") { result.inserted++; log(`✓ KO: ${set.name} (${set.id})`); }
    else if (outcome === "error") result.errors++;
  }

  // ── Chinese sets ──
  log(`Inserting ${CHINESE_SETS.length} Chinese sets...`);
  for (const set of CHINESE_SETS) {
    if (existing.has(set.id)) { result.skipped++; continue; }
    const series = inferSeries(set.id);
    const outcome = await upsertSet(set.id, set.name, series, set.releaseDate || "", "", "", 0);
    if (outcome === "inserted") { result.inserted++; log(`✓ ZH: ${set.name} (${set.id})`); }
    else if (outcome === "error") result.errors++;
  }

  log(`Done. Inserted: ${result.inserted}, Skipped (already existed): ${result.skipped}, Errors: ${result.errors}`);
  return result;
}

export const JAPANESE_SET_SLUGS = JAPANESE_SETS;
export const TOTAL_ASIAN_SETS = JAPANESE_SETS.length + KOREAN_SETS.length + CHINESE_SETS.length;
